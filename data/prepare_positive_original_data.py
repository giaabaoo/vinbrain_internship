...
if __name__ == "__main__":
    ...
    Path("../../dataset/pngs/pos_original_rle_images/train/").mkdir(parents=True, exist_ok=True)
    Path("../../dataset/pngs/pos_original_rle_images/test/").mkdir(parents=True, exist_ok=True)
    Path("../../dataset/annotations/pos_full_rle").mkdir(parents=True, exist_ok=True)
    ...
    for image_id in tqdm(train_keys):
        ...
        # cv2.imwrite("../../dataset/pngs/pos_original_rle_images/train/" + str(image_id) + ".png", image)
    ...
    for image_id in tqdm(test_keys):
        ...
        file_path = dicom_path_dict[image_id]
        # image = process_dicom(file_path)
        # cv2.imwrite("../../dataset/pngs/pos_original_rle_images/test/" + str(image_id) + ".png", image)
    
    
    print("Generating json annotations for each image...")
    with open('../../dataset/annotations/pos_full_rle/train.json', 'w') as f:
        json.dump(train_dict, f)
    
    with open('../../dataset/annotations/pos_full_rle/test.json', 'w') as f:
